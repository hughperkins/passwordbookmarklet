...
function processPassasdf(){
    ...
    confirminput = document.getElementById(ourpwinputnameasdf + 'confirm');
    var masterconfirm = confirminput.value;
   ...
   if(master!=''&&master!=null){
    ...
    for(i=0;i<F.length;i++){
        var E=F[i].elements;
        if(E.id == ourformidasdf) continue;
        for(j=0;j<E.length;j++){
            var D=E[j];
            if(D.type=='password' && D.id != ourpwinputnameasdf && D.id != ourpwinputnameasdf + 'confirm' ){
                D.value=p;
                D.focus();
                g=true;
            }
            if(D.type=='text' && D.id != ourpwinputnameasdf  && D.id != ourpwinputnameasdf + 'confirm'){
...
/*   var master=window.prompt('Enter your master password');*/
   var ourdiv = document.getElementById(ourdivnameasdf);
    var newdivhtml = '<div style="z-index: 100; position: absolute; left: 100px; top: 100px; background-color: #ecc; border: 1px solid black" id="' + ourdivnameasdf + '" ><form  onsubmit="processPassasdf();return false;" id="' + ourformidasdf + '">Master password: <input type="password" id="' + ourpwinputnameasdf + '"/><br />Confirm password:<input type="password" id="' + ourpwinputnameasdf + 'confirm"/><br /><input type="submit" value="Get password" /></form></div>';
    if( ourdiv == null ) {
        document.body.innerHTML = newdivhtml + document.body.innerHTML;
        document.getElementById(ourpwinputnameasdf).focus();
    } else {
        /*document.getElementById(ourdivnameasdf).setAttribute('style','z-index: 100; position: absolute; left: 100px; top: 100px; background-color: #ecc; border: 1px solid black');*/
        /*document.getElementById(ourpwinputnameasdf).value = '';*/
        /*document.getElementById(ourpwinputnameasdf+confirm).value = '';*/
